        ...
        var hasPlayerId = _.every(obj, function(m) {
          ...
          return !angular.isUndefinedOrNull(m.players) && _.every(m.players, function(eachPlayer) {
            ...
            var checkObject = function(parentObj) {
              console.log('checking for', currentObjectKey, 'in', parentObj);
              currentObjectKey = key.indexOf('.') !== -1 ? key.split('.')[level] : key;
              if (angular.isUndefinedOrNull(parentObj[currentObjectKey])) {
                //console.warn(parentObj, currentObjectKey, parentObj[currentObjectKey]);
                throw new Error('FAILED: could not find property \'' + currentObjectKey, '\' in' + parentObj);
                return false;
              } else {
                level += 1;
                if (level < maxLevels) {
                  return !angular.isUndefinedOrNull(parentObj[currentObjectKey]) && (console.log('found key in object, now going deeper', parentObj[currentObjectKey])) && checkObject(parentObj[currentObjectKey]);
                } else {
                  return true;
                }
              }
            };
            return checkObject(eachPlayer);
          });
